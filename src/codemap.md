...
- Exposes the **Hashline tool suite** (line-stable file operations) to the OpenCode runtime via `tool["read"]`, `tool["edit"]`, `tool["patch"]`, and `tool["write"]`, plus `tool["hash-check"]` for preflight validation.
...
- **Tool registration** (wiring only, implementations live outside `src/`):
  ...
  - `hashCheckTool` imported from `../.opencode/tools/hash-check`
- **Implementation layering**:
  - `.opencode/tools/{read,edit,patch,write}.ts` contain the hashline tool implementations registered under built-in-surface names.
  - `.opencode/tools/hash-check.ts` remains the custom preflight validation tool.
- **Routing composition**:
...
3. `hashlinePlugin` returns the plugin contract:
   - `return { ...routingHooks, tool: { "read": readTool, "edit": editTool, "patch": patchTool, "write": writeTool, "hash-check": hashCheckTool } }`
...
- **Tool integration**:
  - `../.opencode/tools/{read,edit,patch,write}` provide the registered built-in-surface hashline handlers.
  - `../.opencode/tools/hash-check` provides the custom preflight validation tool.
